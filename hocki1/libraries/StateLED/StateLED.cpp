...
void StateLED::topOn(HEKF* filter, bool tankDrive, float targheading) {
    uint8_t currState;
    ...
    if(topState == TOP_TANK) {
        setTopColor(TANK_COLOR);
    }
...
void StateLED::bottomOn(HEKF* filter, bool tankDrive, float targheading) {
    uint8_t currState;
    float angvel = filter->x(1, 0);
    float heading = filter->x(0, 0);
    
    if(tankDrive) {
        currState = BOTTOM_TANK;
    }
    else if(aboutEqual(heading, targheading) && angvel < TARG_ANG_VEL) {
    ...
    if(bottomState == BOTTOM_TANK) {
        setBottomColor(TANK_COLOR);
    }
