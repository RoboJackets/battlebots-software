...
input_voltage(1,:) = input(1); %%+ sr .* tt;
input_voltage(2,:) = input(2); %%+ sr .* tt;
...
actual_input(1,:) = input_voltage(1,:);
actual_input(2,:) = input_voltage(2,:);

% fprintf("Actual Input: %d\n", actual_input(1,1));
% fprintf("size of actual_input is %d\n", size(actual_input));

% [yy, ~, ~] = lsim(sys, actual_input, tt, curr_state);
yy = zeros(6, numel(tt)+1);
yy(:, 1) = curr_state.';
for k=2:numel(tt)+1
%     temp = size(yy(:, k-1));
%     %fprintf("yy(:, k-1) is %d\n", temp);
...
%     fprintf("size of actual_input(k-1, :) is %d\n", size(actual_input(:, k-1)));
    ...
    yy_dot = A * yy(:, k-1) + B * actual_input(:, k-1);
    yy(1:4, k) = yy(1:4, k-1) + dt*(yy_dot(1:4));
    yy(5:6, k) = yy_dot(5:6);
    if k==1
        fprintf("actual_input: %f\n", actual_input(:,k-1));
        fprintf("yy: %d\n", yy(:,k-1));
        fprintf("yy_dot: %d\n", yy_dot);
        fprintf("dt: %f\n", dt);
    end
